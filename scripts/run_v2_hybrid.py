"""V3 hybrid: v2 + 2stage but SKIP validation for high-confidence flags (>=0.85)
to preserve TRUE signal lost in v2+2stage."""
...
log = logging.getLogger("v2hybrid")

RAW = ROOT / "data/raw/bulk"
OUT = ROOT / "data/reports/bulk_v2_hybrid"
OUT.mkdir(parents=True, exist_ok=True)

# 10 signal lots (where we know v1 has TRUE/PARTIAL flags)
LOTS = [
...
def main() -> None:
    predictor = OpenAIBaselinePredictorV2(
        ...
        two_stage=True,
        section_gating=True,
        high_conf_skip=0.85,  # skip 2stage for high-conf flags
    )
